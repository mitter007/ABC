        ...
        esData.foreachRDD(rdd -> {
            ...
            rdd.foreachPartition(partition -> {
                ...
                partition.forEachRemaining(record -> {
                    String date = (String) record.get("kpidate");
                    if (date == null) date = "unknown";

                    String dynamicIndex = esIndex + "_" + date;   // 最终索引名

                    // ES 支持在文档里指定 _index 达成动态路由
                    record.put("_index", dynamicIndex);
                ...
                if (!list.isEmpty()) {
                    ...
                    JavaRDD<Map<String, Object>> rddToSave =
                            jsc.parallelize(list, 1);

                    JavaEsSpark.saveToEs(rddToSave, "/_doc");  // index在文档里指定
                }
            });
        });
