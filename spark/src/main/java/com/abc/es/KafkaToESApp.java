...
public class KafkaToESApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ---------- foreachRDD 写入 ES ----------
        esData.foreachRDD((JavaRDD<Map<String, Object>> rdd) -> {
            if (!rdd.isEmpty()) {
                // Optional: 打印批次大小（注意：count 会触发 action）
                long count = rdd.count();
                System.out.println("准备写入 ES，条数 = " + count);

                // 保存到 ES（index name）
                // 推荐写法（ES7.x）：index/_doc 或 直接 index（es-hadoop 会处理）
                String target = esIndex + "/_doc";
                JavaEsSpark.saveToEs(rdd, target);

                System.out.println("写入 ES 完成");
            }
        });
